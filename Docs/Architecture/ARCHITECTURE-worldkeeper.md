...
A retry with the same operation identity and exact prepared binding must recover the original publication rather than create a second child. A successful commit is not erased by a later read/refresh failure; the receipt and refresh outcome are separate facts.
